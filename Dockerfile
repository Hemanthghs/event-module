...
# then log output of file /code/bin/eventd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/eventd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/eventd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/eventd /usr/bin/eventd
