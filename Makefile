...
HTTPS_GIT := github.com/rollchains/eventchain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=eventchain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=eventd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/eventd ./cmd/eventd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/eventd.exe ./cmd/eventd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/eventd
...
draw-deps:
	...
	@goviz -i ./cmd/eventd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c eventchain --local -f chains.yaml
endif
...
set-testnet-configs:
	eventd config set client chain-id localchain-1
	eventd config set client keyring-backend test
	eventd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | eventd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | eventd keys add acc1 --recover`
